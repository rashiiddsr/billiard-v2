...
import { Plus, Search, Edit2, X, Copy } from 'lucide-react';
...
export default function CashierMembersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setTotal(res?.total ?? asArray(res).length);
    } catch {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editId) {
        ...
        setCreatedCredentials(res?.credentials || null);
        ...
        {totalPages > 1 && (
          <div style={{ display: 'flex', justifyContent: 'center', gap: 8, padding: '16px', borderTop: '1px solid var(--color-border-soft)' }}>
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
              <button key={p} className={`btn btn-sm ${p === page ? 'btn-dark' : 'btn-ghost'}`} onClick={() => setPage(p)}>
                {p}
              </button>
            ))}
          </div>
            ...
            {createdCredentials ? (
              <>
                <div className="card-header">
                  <h3 className="card-title" style={{ color: 'var(--color-success)' }}>✅ Member Berhasil Dibuat</h3>
                </div>
