...
export default function ProfilePage() {
  ...
  return (
    <div style={{ maxWidth: 560 }}>
      ...
      {/* Avatar */}
      <div className="card card-padded mb-4" style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
        <div style={{ position: 'relative' }}>
          <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'var(--color-primary)', border: '3px solid var(--color-gold)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, fontWeight: 700, color: 'var(--color-gold)', overflow: 'hidden' }}>
            {user?.profileImageUrl ? <img src={user.profileImageUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : initials}
          </div>
          <label style={{ position: 'absolute', bottom: 0, right: 0, width: 24, height: 24, background: 'var(--color-gold)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', border: '2px solid white' }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 13, color: 'var(--color-text-muted)', textTransform: 'capitalize' }}>{user?.role?.toLowerCase()}</div>
      ...
      {/* Password */}
      <div className="card">
        <div className="card-header" style={{ paddingBottom: 14 }}><h3 className="card-title"><Lock size={15} style={{ display: 'inline', verticalAlign: 'middle', marginRight: 6 }} />Ubah Password</h3></div>
        <div className="card-body" style={{ paddingTop: 0 }}>
          ...
          <button className="btn btn-outline" onClick={handlePasswordChange} disabled={savingPw}>{savingPw ? 'Mengubah...' : 'Ubah Password'}</button>
        </div>
      </div>
    </div>
  );
